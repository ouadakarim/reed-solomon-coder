...
class RSCoder(object):
    ...
    def rs_generator_poly(self, nsym):
        ...
        for i in range(0, nsym):
            g = self.GF.gf_poly_mul(g, [1, self.GF.gf_pow(2, i)])
        return g
    ...
    def rs_encode_msg(self, msg_in, nsym):
        ...
        _, remainder = self.GF.gf_poly_div(msg_in + [0] * (len(gen) - 1), gen)
    ...
    def rs_calc_syndromes(self, msg, nsym):
        ...
        synd = [0] * nsym
        for i in range(0, nsym):
            synd[i] = self.GF.gf_poly_eval(msg, self.GF.gf_pow(2, i))
        return [0] + synd
    ...
    def rs_find_errata_locator(self, e_pos):
        ...
        e_loc = [1]
        for i in e_pos:
            e_loc = self.GF.gf_poly_mul(e_loc, self.GF.gf_poly_add([1], [
                self.GF.gf_pow(2, i), 0]))
        return e_loc
    ...
    def rs_find_error_evaluator(self, synd, err_loc, nsym):
        ...
        """
        _, remainder = self.GF.gf_poly_div(self.GF.gf_poly_mul(synd, err_loc),
                                           ([1] + [0] * (nsym + 1)))
        return remainder
    ...
    def rs_correct_errata(self, msg_in, synd, err_pos):
        ...
        for i, Xi in enumerate(X):
            Xi_inv = self.GF.gf_inverse(Xi)
            err_loc_prime_tmp = []
            for j in range(0, Xlength):
                if j != i:
                    err_loc_prime_tmp.append(
                        self.GF.gf_sub(1, self.GF.gf_mul(Xi_inv, X[j])))
            err_loc_prime = 1
            for coef in err_loc_prime_tmp:
                err_loc_prime = self.GF.gf_mul(err_loc_prime, coef)

            y = self.GF.gf_poly_eval(err_eval[::-1], Xi_inv)
            y = self.GF.gf_mul(self.GF.gf_pow(Xi, 1), y)

            # Compute the magnitude
            magnitude = self.GF.gf_div(y, err_loc_prime)
            E[err_pos[i]] = magnitude
        msg_in = self.GF.gf_poly_add(msg_in, E)
        return msg_in
    ...
    def rs_find_error_locator(self, synd, nsym, erase_loc=None, erase_count=0):
        ...
        for i in range(0, nsym - erase_count):
            ...
            for j in range(1, len(err_loc)):
                delta ^= self.GF.gf_mul(err_loc[-(j + 1)], synd[K - j])

            old_loc = old_loc + [0]

            if delta != 0:
                if len(old_loc) > len(err_loc):
                    new_loc = self.GF.gf_poly_scale(old_loc, delta)
                    old_loc = self.GF.gf_poly_scale(err_loc,
                                                    self.GF.gf_inverse(delta))
                    err_loc = new_loc

                err_loc = self.GF.gf_poly_add(err_loc,
                                              self.GF.gf_poly_scale(old_loc,
                                                                    delta))
    ...
    def rs_find_errors(self, err_loc, nmess):  # nmess is len(msg_in)
        ...
        for i in range(nmess):
            if self.GF.gf_poly_eval(err_loc, self.GF.gf_pow(2, i)) == 0:
    ...
    def rs_forney_syndromes(self, synd, pos, nmess):
        ...
        for i in range(0, len(pos)):
            x = self.GF.gf_pow(2, erase_pos_reversed[i])
            for j in range(0, len(fsynd) - 1):
                fsynd[j] = self.GF.gf_mul(fsynd[j], x) ^ fsynd[j + 1]
        return fsynd
